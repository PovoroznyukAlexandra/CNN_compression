...
from vgg16_covid19_test import test_accuracy, model_compression_ratio
...
class RSVDLinear(nn.Module):
    def __init__(self, original_layer, layer_name, rank):
        ...
        self.Vt = nn.Parameter(torch.from_numpy(Vt))
...
def compress_vgg16_rsvd(original_model):
    compressed_model = copy.deepcopy(original_model)
    
    rsvd_ranks = {'linear1': 10, 'linear2': 10}
...
if __name__ == '__main__':
    ...
    rsvd_compressed_model = compress_vgg16_rsvd(model)
    rsvd_compressed_model = rsvd_compressed_model.to(device)
    torch.save(rsvd_compressed_model.state_dict(), "weights/rsvd_vgg16_covid19.pth")
